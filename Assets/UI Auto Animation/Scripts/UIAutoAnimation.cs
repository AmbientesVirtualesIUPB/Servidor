...
public class UIAutoAnimation : MonoBehaviour
{
    ...
    public bool iniciarEnExit = true;

    public void Awake()
    {
        GetComponentsList();
        if (iniciarEnExit) SetExitFinalState();
    }
    ...
    private void GetComponentsList()
    {
        ...
        GetComponentInHierarchy_DepthFirst(transform, typeof(TextMeshProUGUI), typeof(Image), typeof(CanvasGroup));
        ...
        for (int i = 0; i < componentList.Count; i++)
        {
            ...
            // A�adir esto dentro del bucle for i < componentList.Count
            var cg = componentList[i].GetComponent<CanvasGroup>();
            if (cg != null)
            {
                if (!componentList.Contains(cg))
                {
                    componentList[i] = cg;
                }
            }
        ...
        for (int i = 0; i < componentList.Count; i++)
        {
            ...
            else if (componentList[i] is CanvasGroup)
            {
                originalAlpha[i] = ((CanvasGroup)componentList[i]).alpha;
            }
        }
    ...
    private void SetColorAlpha(Component component, float value)
    {
        ...
        else if (component is Image)
        {
            ...
            ((Image)component).raycastTarget = alpha > 0.5f;
        }
